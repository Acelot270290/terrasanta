...
class modInstallerScs {
	...
	static private function _getExtendModules($locations) {
		...
		if(!empty($modulesList)) {
			foreach($modulesList as $mod) {
				$modData = $isExtendModPath ? $mod : utilsScs::xmlNodeAttrsToArr($mod);
				array_push($modules, $modData);
			}
			if(empty($modules))
				errorsScs::push(__('No modules were found in installation file', SCS_LANG_CODE), errorsScs::MOD_INSTALL);
			else
				return $modules;
		} else
			errorsScs::push(__('No installation file were found', SCS_LANG_CODE), errorsScs::MOD_INSTALL);
		return false;
	}
    /**
    ...
    static public function check($extPlugName = '') {
		...
		if($modules = self::_getExtendModules($locations)) {
			foreach($modules as $m) {
				if(!empty($m)) {
					//If module Exists - just activate it, we can't check this using frameScs::moduleExists because this will not work for multy-site WP
					if(frameScs::_()->getTable('modules')->exists($m['code'], 'code') /*frameScs::_()->moduleExists($m['code'])*/) {
						self::activate($m);
					} else {                                           //  if not - install it
						if(!self::install($m, $locations['plugDir'])) {
							errorsScs::push(sprintf(__('Install %s failed'), $m['code']), errorsScs::MOD_INSTALL);
						}
					}
				}
			}
    ...
    static public function deactivate() {
		...
		if($modules = self::_getExtendModules($locations)) {
			foreach($modules as $m) {
				if(frameScs::_()->moduleActive($m['code'])) { //If module is active - then deacivate it
					if(frameScs::_()->getModule('options')->getModel('modules')->put(array(
						'id' => frameScs::_()->getModule($m['code'])->getID(),
						'active' => 0,
					))->error) {
    ...
    static public function activate($modDataArr) {
		if(!empty($modDataArr['code']) && !frameScs::_()->moduleActive($modDataArr['code'])) { //If module is not active - then acivate it
			if(frameScs::_()->getModule('options')->getModel('modules')->put(array(
    ...
    static public function uninstall() {
		...
		if($modules = self::_getExtendModules($locations)) {
			foreach($modules as $m) {
				...
				frameScs::_()->getModule('options')->getModel('modules')->delete(array('code' => $m['code']));
				utilsScs::deleteDir(SCS_MODULES_DIR. $m['code']);
			}
		}
    }
